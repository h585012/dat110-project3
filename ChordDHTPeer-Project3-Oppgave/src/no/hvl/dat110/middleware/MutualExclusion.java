...
public class MutualExclusion {
	...
	public boolean doMutexRequest(Message message, byte[] updates) throws RemoteException {
		...
		List<Message> dobbelt = removeDuplicatePeersBeforeVoting();
		// multicast the message to activenodes (hint: use multicastMessage)
		multicastMessage(message, dobbelt);
	...
	private void multicastMessage(Message message, List<Message> activenodes) throws RemoteException {
		...
		for(Message melding : activenodes) {
			// obtain a stub for each node from the registry
			NodeInterface stub = Util.getProcessStub(melding.getNodeIP(), melding.getPort());
			// call onMutexRequestReceived()
			stub.onMutexRequestReceived(melding);
		}
	}
	...
	public void doDecisionAlgorithm(Message message, List<Message> queue, int condition) throws RemoteException {
		...
		switch(condition) {
			...
			case 2: {
				...
				boolean lowestClock = message.getClock() < node.getMessage().getClock();
				boolean sameClock = message.getClock() <= node.getMessage().getClock();
	...
	public void multicastReleaseLocks(Set<Message> activenodes) {
		...
		// iterate over the activenodes
		for(Message meldinger : activenodes) {
			NodeInterface node = Util.getProcessStub(meldinger.getNodeIP(), meldinger.getPort());
		}
		try {
			node.releaseLocks();
	...
	private boolean areAllMessagesReturned(int numvoters) throws RemoteException {
		...
		boolean sammenligne = queueack.size() == numvoters;
		// clear the queueack
		queueack.clear();
